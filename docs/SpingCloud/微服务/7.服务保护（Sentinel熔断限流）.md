...
要想**让Sentinel监听得到openfeign的rpc调用**，还要设置一个额外配置，让openfeign再额外走一下Sentinel的监听链路：
...
注意这个不开openfeign的请求Sentinel是感知不到的！

> **那为什么外部请求可以被Sentinel感知到？**
>
> 当你引入 `spring-cloud-starter-alibaba-sentinel` 后，Sentinel 会**自动注册**一个 Spring MVC 拦截器：
>
> ```
> ┌─────────────────────────────────────────────────────────┐
> │              外部 HTTP 请求流程（自动埋点）                 │
> ├─────────────────────────────────────────────────────────┤
> │  1. 用户请求 → http://api/order/create                   │
> │         ↓                                               │
> │  2. Tomcat/Servlet Container                            │
> │         ↓                                               │
> │  3. [SentinelWebInterceptor] ← 自动生效，无需配置          │
> │         ↓ 自动埋点：资源名 = URL 路径                      │
...
> ```
>
> | 对比项       | 外部 HTTP 请求              | OpenFeign RPC 调用                  |
> | :----------- | :-------------------------- | :---------------------------------- |
> | **适配器**   | `SentinelWebInterceptor`    | `SentinelFeign`                     |
> | **自动配置** | ✅ 默认开启                  | ❌ 默认关闭                          |
> | **触发条件** | 引入 Sentinel 依赖即可      | 需 `feign.sentinel.enabled=true`    |
> | **资源名**   | URL 路径（如 `/api/order`） | `方法:路径`（如 `GET:/users/{id}`） |
> | **设计原因** | Web 项目几乎都用 MVC        | Feign 是可选组件                    |
