...
| 对比维度          | 方案一 (Redis+ 读写扩散) | 方案二 (时间索引)   |
| :---------------- | :----------------------- | :------------------ |
| **架构复杂度**    | ⭐⭐⭐ 高                   | ⭐ 低                |
| **写性能 (单聊)** | ⭐⭐⭐⭐ 极快                | ⭐⭐⭐ 快              |
| **写性能 (群聊)** | ⭐⭐⭐⭐ 快 (读扩散)         | ⭐⭐ 慢 (写扩散)      |
| **读性能**        | ⭐⭐⭐⭐ Redis 内存读取      | ⭐⭐⭐ 数据库查询      |
| **存储成本**      | ⭐⭐ Redis 内存贵          | ⭐⭐⭐⭐ 磁盘便宜       |
| **数据一致性**    | ⭐⭐ 需同步机制            | ⭐⭐⭐⭐ 天然一致       |
| **历史消息查询**  | ⭐⭐ 需关联查询            | ⭐⭐⭐⭐ 原生支持       |
| **消息撤回**      | ⭐⭐ 复杂                  | ⭐⭐⭐⭐ 简单           |
| **未读计数**      | ⭐⭐⭐⭐ 列表长度即未读      | ⭐⭐ 需额外维护       |
| **离线推送**      | ⭐⭐⭐⭐ Redis 天然适合      | ⭐⭐ 需额外机制       |
| **故障恢复**      | ⭐⭐ Redis 可能丢数据      | ⭐⭐⭐⭐ 持久化安全     |
| **运维成本**      | ⭐⭐ 需维护 Redis 集群     | ⭐⭐⭐⭐ 单一数据库     |
| **扩展性**        | ⭐⭐⭐ Redis 分片复杂       | ⭐⭐⭐⭐ 数据库分表成熟 |

| 项目阶段     | 推荐方案            | 理由                 |
| :----------- | :------------------ | :------------------- |
| **MVP/初创** | 方案二 (时间索引)   | 快速上线，维护简单   |
| **成长期**   | 方案二 + Redis 缓存 | 平衡性能与成本       |
| **成熟期**   | 方案一 (混合架构)   | 支撑高并发，优化体验 |
| **超大规模** | 方案一 + 分库分表   | 千万级 DAU 必备      |

| 方案                | 核心优势             | 核心劣势         | 推荐场景     |
| :------------------ | :------------------- | :--------------- | :----------- |
| **Redis+ 读写扩散** | 推送快、未读计数简单 | 架构复杂、成本高 | 大规模 IM    |
| **时间索引**        | 架构简单、数据一致   | 群聊写性能差     | 中小规模 IM  |
| **混合方案**        | 兼顾性能与成本       | 实现复杂度高     | 生产环境首选 |

**最佳实践**：初期用方案二快速验证，用户增长后逐步引入 Redis 做热数据缓存，最终演进为混合架构。
