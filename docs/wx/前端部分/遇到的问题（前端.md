...
### 注意主线程的请求和渲染线程的axios存在区别

比方说请求头里面的userAgent：

1. **主进程**：
   - 运行在 Node.js 环境中
   - 使用 axios 等库发起请求时，默认 User-Agent 是 Node.js 提供的字符串（例如 `axios/x.x.x`）
   - 不会自动携带浏览器中常见的 User-Agent 信息
2. **渲染进程**：
   - 运行在 Chromium 浏览器环境中
   - 发起请求时自动使用浏览器的 User-Agent，通常包含 Electron 和操作系统的相关信息

所以对于验证消息还是自行获取的动态数据好，当然如果就想要在主线程中使用userAgent来作为验证消息，也可以ipc通信，将渲染线程获取到的传给主线程，主线程用eletron的store存一下，每次需要发了从里面取，7

而且，如果需要为主线程的axios配备请求前拦截器等，记得给主线程自己配一个（或者干脆用渲染线程的）



### 不要乱用removeAllListeners来消除监听器！！！

子模块的removeAllListeners是影响的到父模块的，一不注意就会把**全局监听器**给消掉！

**<u>对监听器的范围一定在一开始用的时候就确认好！</u>**这次遇到的问题：

准备实现一个WebRTC的音视频通话，那么这时候不管处于哪个页面，我们都需要监听主线程的`wsclient`的`receiveMessage`信号，这时候我在`main.vue`准备了监听全局`receiveMessage`的监听函数，但是在`chat.vue`里面的`onUnmount`函数里面却使用了`removeAllListeners`去消除主窗口渲染线程中所有监听了`receiveMessage`的监听器，这就导致了**一旦切换子页面就会导致监听器失效**！

```js
window.ipcRenderer.removeAllListeners('receiveMessage') // 尽量不要这样！！！！
```

可以的话尽量只消除**一个**监听器方法：

```js
window.ipcRenderer.removeListener('receiveMessage', handleWebSocketMessage)
```



### Eletron的麦克风/摄像头只能和其他应用抢

没有办法一起用的，只能挪到另一台电脑上完成webrtc的测试



### 本地视频流由于异步的问题没能和video块绑定

在原先的call.vue中的几个媒体流处理的存在一定问题：

```
acceptCall 执行时:
├── callState = 'calling'  ← 此时 localVideo 元素还没渲染
├── getLocalStream() 成功
├── 尝试绑定 localVideo.value → null  ← 失败！
└── 后续 callState 才变为 'connected'
```

这里并不是因为getLocalStream不存在，相反，是video的问题：

```vue
<div v-else-if="callState == 'connected'" class="connected-state">
  <video v-if="callType === 'video'" ref="localVideo" ...></video>
</div>
```

这里callState不是connected的时候localVideo是尚未开始渲染的，这就会导致localVideo.value无法拿到，我们只能等待callState切换的时候再尝试将本地的视频流和video模块进行绑定，要实现起来也很简单，watch监听callState即可，若为connected则再次尝试进行绑定：

```js
watch(callState, (newCallState) => {
  console.log('callState 变化:', newCallState)
  if (newCallState === 'connected') {
    nextTick(() => {
      console.log('connected 状态下绑定本地视频')
      console.log('localVideo ref:', localVideo.value)
      ...
      if (localVideo.value && WebRTCService.localStream) {
        ...
        console.log('本地视频绑定成功, tracks:', WebRTCService.localStream.getTracks().map(t => t.kind))
        ...
        console.warn('无法绑定本地视频:', {
          hasLocalVideo: !!localVideo.value,
